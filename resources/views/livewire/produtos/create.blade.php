<div>
    <form wire:submit.prevent="save">
        ...
        <x-card title="Cadastro de produtos">
            ...
            <x-input wire:model.defer="price" label="Preço de Custo:" type="number" placeholder="Custo" name="price" />
            @error('price') <span class="error text-red-500" >{{ $message }}</span> @enderror

            <x-input wire:model.defer="lucro" label="Lucro (%):" type="number" placeholder="Percentual de Lucro" name="lucro" />
            @error('lucro') <span class="error text-red-500" >{{ $message }}</span> @enderror
